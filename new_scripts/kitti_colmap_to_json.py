...
def colmap_to_json(cameras_path: Path, images_path: Path, output_dir: Path, camera_model: CameraModel) -> int:
    ...
    for _, im_data in images.items():
        if im_data.name[:2] != "00":
            continue
        im_name = im_data.name[3:]
        ...
        name = Path(f"./images/image_00/data_rect/{im_name}")
        ...
        file_name = "kitti00_" + im_name
        depth_path = os.path.join("./depth_priors", file_name)
...
def main():
    output_dir = Path("/scratch_net/biwidl208/yuthan/wisp_data/KITTI-360")
    colmap_dir = output_dir
    camera_type = "other"
    camera_model = CAMERA_MODELS[camera_type]
    num_matched_frames = colmap_to_json(
        cameras_path=colmap_dir / "sparse" / "orientation_aligned" / "cameras.bin",
        images_path=colmap_dir / "sparse" / "orientation_aligned" / "images.bin",
