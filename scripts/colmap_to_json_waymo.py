...
def colmap_to_json(cameras_path: Path, images_path: Path, output_dir: Path, camera_model: CameraModel) -> int:
    ...
    for _, im_data in images.items():
        ...
        name = Path(f"./colmap/images/{im_data.name}")

        frame = {
            "file_path": str(name),
            "transform_matrix": c2w.tolist(),
        }
        if (cameras[im_data.camera_id].width == 1096 and 
            cameras[im_data.camera_id].height == 1217):
            frames.append(frame)
    out = {
...
def main():
    output_dir = Path("/srv/beegfs02/scratch/bdd100k/data/sfm/nerfstudio/data/waymo/block_0")
